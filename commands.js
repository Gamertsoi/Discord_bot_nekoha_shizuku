...
async function handleMsg(message, args) {
  ...
  try {
    ...
    try { await message.channel.send('Failed to send message. Check permissions and channel type.'); } catch { /* ignore */ }
  }
}
...
async function handleMsgRole(message, args, reactionRoleMap, saveMappings) {
  ...
  if (args.length >= 1 && args[0].toLowerCase() === 'remove') {
    ...
    try { await saveMappings(); } catch (saveErr) { console.warn('Failed to save mappings after removal:', saveErr?.message || saveErr); }
  ...
  try { await saveMappings(); } catch (saveErr) { console.warn('Failed to save mappings:', saveErr?.message || saveErr); }
...
async function handleClr(message, args) {
  ...
  if (countArg === 'all') {
    ...
    try {
      ...
      try { await message.channel.send(`Cleared messages in <#${targetChannel.id}>. Total deleted (approx): ${totalDeleted}`); } catch (replyErr) { console.debug('Could not send clear confirmation:', replyErr?.message || replyErr); }
      return;
    } catch (err) {
      console.error('Error clearing all messages:', err);
      try { await message.channel.send('Failed to clear messages. Check my permissions and try again.'); } catch { /* ignore */ }
      return;
    }
  }
  ...
  try {
    ...
    try { await message.channel.send(`Deleted approximately ${totalDeleted} message(s) from <#${targetChannel.id}>.`); } catch (replyErr) { console.debug('Could not send deletion confirmation:', replyErr?.message || replyErr); }
    return;
  } catch (err) {
    console.error('Error deleting messages:', err);
    try { await message.channel.send('Failed to delete messages. Check my permissions and try again.'); } catch { /* ignore */ }
    return;
  }
}
